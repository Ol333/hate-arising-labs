...
* Написать программу, которая бы считывала с клавиатуры число $А_{16}$ в шестнадцатеричном формате и выводила значение параметров $Р_1$, $Р_2$, $Р_3$, ($Р_4$).
...
* То же, что и в предыдущем пункте, только считать $А_{16}$ из бинарного [файла]({{ site.baseurl }}/files/TIPiS/7lab_input3.bin).
