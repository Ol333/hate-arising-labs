...
<div class="card border-primary mb-2" style="max-width: 50rem;">
  <div class="card-body">
    <img src="{{ site.baseurl }}/artificial-intelligence/pattern-recognition/labs/lab3_data/images1/SSD_architecture.svg"
        alt="architecture" focusable="false" width="100%"
...
<div class="card border-primary mb-2" style="max-width: 50rem;">
  <div class="card-body">
    <img src="{{ site.baseurl }}/artificial-intelligence/pattern-recognition/labs/lab3_data/images1/архитектура.webp"
        alt="SSD (вверху) против YOLO (внизу)" focusable="false" width="100%"
        class="d-block user-select-none" />
  </div>
</div>
...
Функция потерь состоит из двух членов: $L_{conf}$ и $L_{loc}$, где $N$ — это количество стандартных рамок, которые были сопоставлены с реальными объектами. Эти потери взвешиваются и комбинируются, чтобы обучить модель как можно точнее распознавать объекты.

Потеря локализации

$ L_{loc}(x, l, g) = \sum_{i \in \text{Pos}} \sum_{m \in \{cx, cy, w, h\}} x_{ij}^k {smooth}_{L_1}(l_i^m - \hat{g}_j^m) $
...
<div class="card border-primary mb-2" style="max-width: 50rem;">
  <div class="card-body">
    <img src="{{ site.baseurl }}/artificial-intelligence/pattern-recognition/labs/lab3_data/images1/image-3.png"
        alt="Некоторые результаты обнаружения в наборе данных MS COCO" focusable="false" width="100%"
        class="d-block user-select-none" />
  </div>
</div>


*Последняя версия: Тимура Д.С.*