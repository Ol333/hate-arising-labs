...
* Обучить три сети и объединить:
  ...
  * Объединить три предварительно обученные сети ($^1 x → T$, $^2 x → \varphi$, $^3 (T, \varphi) → Y=f(x)$) в одну сложную [*](https://keras.io/api/layers/merging_layers/concatenate/). Вывести результат.
