...
Если она близка к единице, то модель хорошообъясняет данные,
...
Если же эти объекты являются выбросами (то есть значение целевой переменной на них либо ошибочно, либоотносится к другому распределению и должно быть проигнорировано), то такая расстановка акцентов
...
В задачах прогнозирования обычно измеряется относительная
ошибка:
...
Градиент является направлением наискорейшего роста функции, а антиградиент (т.е. $−∇f$) — направлением наискорейшего убывания. Это ключевое свойство
градиента, обосновывающее его использование в методах оптимизации.

Докажем данное утверждение.
Пусть $v ∈ \Bbb R^d$ — произвольный вектор, лежащий на единичной сфере: $ \Vert v \Vert = 1.$
Пусть $x_0 ∈ \Bbb R^d$ — фиксированная точка пространства.
Скорость роста функции в точке $x_0$ вдоль вектора $v$ характеризуется производной
по направлению $\frac{∂f}{∂v}$:

$ \frac{∂f}{∂v} = \frac{d}{dt}f(x_{0,1} + tv_1 ,... , x_{0,d} + tv_d) \vert _{t=0} $ .

Из курса математического анализа известно, что данную производную сложной
функции можно переписать следующим образом:

$ \frac{∂f}{∂v} = \sum_{j=1}^d \frac{∂f}{∂x_j} \frac{d}{dt} (x_{0,j} + tv_j) = \sum_{j=1}^d \frac{∂f}{∂x_j} v_j = \langle ∇f, v \rangle $ .

Распишем скалярное произведение:

$ \langle ∇f, v \rangle = ‖∇f‖‖v‖ \cosφ=‖∇f‖ \cosφ $ ,

где $φ$ — угол между градиентом и вектором $v$. Таким образом, производная по направлению будет максимальной, если угол между градиентом инаправлением равен
нулю, и минимальной, если угол равен 180 градусам. Иными словами, производная
по направлению максимальна вдоль градиента и минимальна вдоль антиградиента.

Покажем теперь, что градиент ортогонален линиям уровня.
Пусть $x_0$ — некоторая точка, $S(x_0) = \\{ x∈ \Bbb R^d \vert f(x) = f(x_0) \\}$ — соответствующая линия уровня.
Разложим функцию в ряд Тейлора на этой линии в окрестности $x_0$ :

$ f(x_0 + ε) = f(x_0) + \langle ∇f, ε \rangle + o(‖ε‖) $ ,

где $x_0 + ε ∈ S(x_0)$  Поскольку $f(x_0 + ε) = f(x_0)$ (как-никак, это линия уровня),
получим

$ \langle ∇f, ε \rangle = o(‖ε‖) $ .

Поделим обе части на $‖ε‖$:

$ \langle ∇f, \frac{ε}{‖ε‖} \rangle = o(1) $ .

Устремим $‖ε‖$ к нулю. При этом вектор $\frac{ε}{‖ε‖}$ будет стремится к касательной к линии
уровня в точке $x_0$. В пределе получим, что градиент ортогонален этой касательной.
...
Соответственно, будет логично стартовать из некоторой точки, сдвинуться в сторону антиградиента, пересчитать антиградиент и снова сдвинуться в его сторону и т.д. 
апишем это более формально. Пусть $w^{(0)}$ — начальный набор параметров (например, нулевой или сгенерированный из некоторого случайного распределения).
...
модели на очень больших выборках: можно считывать объекты сдиска по одному,
