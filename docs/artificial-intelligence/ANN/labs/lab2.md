...
$ ω_{ij}^{t+1}=ω_{ij}^t + \frac {(y_i - \sum{j=1}^n ( ω_{ij}^t x_{ij} )}{\sum_{j=1}^n ω_{ij}^t }$ <a id="eq_1">(1)</a>
...
$ω_{ij}^{t+1}=ω_{ij}^t-α (∂(1/2(〖(\sum(j=1)^n (ω_{ij}^t x_{ij}-y_i))^2)))/(∂x_i )$

Получаем формулу для обновления весов нейронов (для `NeuralNetwork.fit_2()`):

$ω_{ij}^{t+1}=ω_{ij}^t-α(\sum{j=1}^n (ω_{ij}^t x_{ij} )-y_i )*x_{ij} $	<a id="eq_4">(4)</a>
